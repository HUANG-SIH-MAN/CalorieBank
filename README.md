...
- **🔐 隱私與安全**：所有數據儲存在本地 `AsyncStorage`（網頁版）或 SQLite（原生版），API Key 加密儲存於 `SecureStore`，資料完全留在您的裝置。
...
- **Storage**: `AsyncStorage` & `expo-secure-store`（網頁）；`expo-sqlite`（原生）
